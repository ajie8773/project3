...
class Augment:
    ...
    def flip_and_rotate(self, MASK, IMG, img_edge, label_edge):
        # h,w,c
        ...
        if hflip:
            ...
            img_edge = img_edge[:, ::-1]
            label_edge = label_edge[:, ::-1]
        # if vflip:
        #    MASK, IMG = MASK[::-1, :], IMG[::-1, :, :]
        if rot90:
            MASK, IMG = MASK.transpose(1, 0), IMG.transpose(1, 0, 2)
            img_edge = img_edge.transpose(1, 0)
            label_edge = label_edge.transpose(1, 0)

        return MASK.copy(), IMG.copy(), img_edge.copy(), label_edge.copy()