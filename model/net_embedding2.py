...
class Gate(nn.Module):
    def __init__(self, dim, embed_dim, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
    ...
    def forward(self, fea, embedding):
        ...
        feat = feat + fea
        feat = self.mlp2(self.norm2(feat))
        return feat

class EmbbdeingCrossFormer(nn.Module):
    def __init__(self, in_dim, embed_dim):
        super(EmbbdeingCrossFormer, self).__init__()
        decoder_layer = TransformerDecoderLayer(embed_dim, nhead=8, dim_feedforward=4*embed_dim,
    ...
    def forward(self, x, query_embed):
        x = self.mlp(self.norm(x)).transpose(0, 1)
...
class Net(nn.Module):
    def __init__(self, training=True):
        ...
        self.embbdeing1 = EmbbdeingCrossFormer(in_dim=320, embed_dim=embed_dim)
        self.gate1 = Gate(dim=320, embed_dim=embed_dim)
        self.embbdeing2 = EmbbdeingCrossFormer(in_dim=128, embed_dim=embed_dim)
        self.gate2 = Gate(dim=128, embed_dim=embed_dim)
        self.embbdeing3 = EmbbdeingCrossFormer(in_dim=64, embed_dim=embed_dim)
        self.gate3 = Gate(dim=64, embed_dim=embed_dim)
