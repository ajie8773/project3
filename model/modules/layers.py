...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        projected_query = self.query_conv(x).reshape(batch_size, -1, axis).permute(0, 2, 1)
        projected_key = self.key_conv(x).reshape(batch_size, -1, axis)

        attention_map = torch.bmm(projected_query, projected_key)
        attention = self.softmax(attention_map)
        projected_value = self.value_conv(x).reshape(batch_size, -1, axis)
