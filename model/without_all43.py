...
from model.SelfTransformer import Transformer as SelfTransformerEncoder
...
from model.PVT_V2 import pvt_v2_b0
...
class CRM(nn.Module):
    ...
    def forward(self, x, gloab_pred):
        B, L, C = x.shape
        gloab_pred = F.adaptive_avg_pool2d(self.sigmoid(gloab_pred), 1).reshape(B, -1, 1)
        x = gloab_pred.repeat(1,1,self.dim) * x   #+x
        return x

class CRM2(nn.Module):
    def __init__(self, dim, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super(CRM2, self).__init__()
        self.dim = dim
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x, gloab_pred):
        B, L, C = x.shape
        gloab_pred = F.adaptive_avg_pool2d(self.sigmoid(gloab_pred), 1).reshape(B, -1, 1)
        gloab_pred = gloab_pred.repeat(1,1,self.dim)

        q = self.q(x).reshape(B, L, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(gloab_pred).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(gloab_pred).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        pred_tamper_feat = (attn @ v).transpose(1, 2).reshape(B, L, C)
        pred_tamper_feat = self.proj(pred_tamper_feat)
        pred_tamper_feat = self.proj_drop(pred_tamper_feat)

        pred_tamper_feat = pred_tamper_feat + x
        return pred_tamper_feat


class ARM(nn.Module):
    def __init__(self,img_size, dim, pred_out_channel, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.conv_glb = nn.Conv2d(pred_out_channel, dim, kernel_size=3, padding=1, bias=True)
        self.conv_fuse = nn.Sequential(nn.Conv2d(2 * dim, dim, kernel_size=3, padding=1, bias=True),
                                       nn.BatchNorm2d(dim),
                                       ...
                                       nn.BatchNorm2d(dim),
                                       nn.LeakyReLU(inplace=True))
        self.conv_p1 = nn.Linear(dim, 1)

    def forward(self, x, pred_out):
        B, L, C = x.shape
        feat = x.transpose(1, 2).reshape(B, C, int(L**0.5), int(L**0.5))
        gloab_pred = F.pixel_unshuffle(pred_out, pred_out.shape[-1] // int(L**0.5))
        gloab_pred = self.conv_glb(gloab_pred)
        feat = self.conv_fuse(torch.cat([gloab_pred, feat], dim=1))


        pred_out = F.adaptive_avg_pool2d(pred_out, 1).reshape(B, 1, -1)#.transpose(1, 2)
        pred_out = pred_out.repeat(1, 1, self.dim)

        q = self.q(feat.reshape(B, C,-1).transpose(1, 2)).reshape(B, L, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = self.k(pred_out).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        v = self.v(pred_out).reshape(B, 1, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        pred_tamper_feat = pred_tamper_feat + x
        p1 = self.conv_p1(pred_tamper_feat).transpose(1, 2).reshape(B, 1,int(L**0.5), int(L**0.5))
        p1 = F.interpolate(p1, size=(self.img_size, self.img_size))
        return p1, pred_tamper_feat
...
class Net(nn.Module):
    def __init__(self, training=True):
        ...
        self.encoder = pvt_v2_b0()
        self.img_size = 512
        self.encoder.load_state_dict(torch.load('model/pretrain/pvt_v2_b0.pth', map_location='cpu'), strict=False)
        self.fuse32_16 = decoder_module(dim=256, dim2=160, img_size=self.img_size, ratio=16,
                                        kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
        self.fuse16_08 = decoder_module(dim=160, dim2=64, img_size=self.img_size, ratio=8, kernel_size=(3, 3), stride=(2, 2),
                                        padding=(1, 1))
        self.fuse08_04 = decoder_module(dim=64, dim2=32, img_size=self.img_size, ratio=4, kernel_size=(3, 3), stride=(2, 2),
                                        padding=(1, 1))
        self.decoder_to_1 = decoder_module(dim=32, dim2=32, img_size=self.img_size, ratio=1, kernel_size=(7, 7), stride=(4, 4),
                                           padding=(2, 2), fuse=False)

        self.linear_32 = nn.Linear(256, 1)
        self.linear_16 = nn.Linear(160, 1)
        self.linear_08 = nn.Linear(64, 1)
        self.linear_04 = nn.Linear(32, 1)
        self.linear_01 = nn.Linear(32, 1)

        self.CRM_32 = ARM(dim=256, pred_out_channel=1024, img_size=self.img_size)
        self.CRM_16 = ARM(dim=160, pred_out_channel=256, img_size=self.img_size)
        self.CRM_08 = ARM(dim=64, pred_out_channel=64, img_size=self.img_size)
        self.CRM_04 = ARM(dim=32, pred_out_channel=16, img_size=self.img_size)

        self.tf = nn.Sequential(
            nn.Linear(512, 256),
            nn.GELU(),
            nn.Linear(256, 256),
            Token_performer(dim=256, in_dim=256, kernel_ratio=0.5)
        )
        self.proj = nn.Linear(256, 1)

        self.up2 = Up(dim=256, output_size=self.img_size//16, kernel_size=3, stride=(2, 2), padding=(1, 1))
        self.down_2 = nn.Sequential(nn.BatchNorm2d(64),
                                               nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1),
                                               nn.LeakyReLU())
        self.down_4 = nn.Sequential(nn.BatchNorm2d(32),
                                               nn.Conv2d(32, 32, kernel_size=7, stride=4, padding=3),
                                               nn.LeakyReLU())
        self.encoder_merge = nn.Sequential(nn.BatchNorm2d(512),
                      nn.Conv2d(512, 256, kernel_size=3, padding=1, bias=True),
                      nn.LeakyReLU())
        self.encoder_tf = SelfTransformerEncoder(depth=2,
                                         num_heads=1,
                                         embed_dim=256,
                                         mlp_ratio=3,
                                         num_patches=(self.img_size // 16)*(self.img_size // 16))
        ...
        # transformer_config = [[2, 1, 256, 3, (self.img_size // 32)*(self.img_size // 32)],
        #                    [2, 1, 160, 3, (self.img_size // 16)*(self.img_size // 16)],
        #                    [2, 1, 64, 3, (self.img_size // 8)*(self.img_size // 8)],
        #                    [2, 1, 32, 3, (self.img_size // 4)*(self.img_size // 4)]]
        # self.transformer = nn.ModuleList([SelfTransformerEncoder(depth=d,
        #                                               num_heads=n,
        #                                               embed_dim=e,
        #                                               mlp_ratio=m,
        #                                               num_patches=p) for d, n, e, m, p in transformer_config])
    ...
    def forward(self, input):
        ...
        up_feat32 = self.up2(feat_32).transpose(1,2).reshape(B, 256, self.img_size // 16, self.img_size // 16)
        down_feat08 = self.down_2(feat_08.transpose(1, 2).reshape(B, 64, self.img_size // 8, self.img_size // 8))
        down_feat04 = self.down_4(feat_04.transpose(1, 2).reshape(B, 32, self.img_size // 4, self.img_size // 4))
        out = self.encoder_merge(torch.cat([feat_16.transpose(1,2).reshape(B, 160, self.img_size // 16, self.img_size // 16),
                   up_feat32,
                   down_feat08,
                   down_feat04], dim=1)).reshape(B, 256, -1).transpose(1, 2)
        out = self.encoder_tf(out)
        out = self.encoder_pred(out).transpose(1, 2).reshape(B, 1, self.img_size // 16, self.img_size // 16)
        pred_out = F.interpolate(out, size=(input.size(2), input.size(3)), mode="bilinear", align_corners=True)

        # feat_32, feat_16, feat_08, feat_04 = [tf(o, peb) for tf, o, peb in zip(self.transformer,
        #                                                                   [feat_32, feat_16, feat_08, feat_04],
        #                                                                   [False, False, False,
        #                                                                    False])]

        pred_32, feat_32 = self.CRM_32(feat_32, pred_out)
        fusion_16 = self.fuse32_16(feat_32, feat_16)
        pred_16, fusion_16 = self.CRM_16(fusion_16, pred_out)
        fusion_08 = self.fuse16_08(fusion_16, feat_08)
        pred_08, fusion_08 = self.CRM_08(fusion_08, pred_out)
        fusion_04 = self.fuse08_04(fusion_08, feat_04)
        pred_04, fusion_04 = self.CRM_04(fusion_04, pred_out)
