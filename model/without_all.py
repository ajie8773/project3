...
class ARM(nn.Module):
    def __init__(self, upperfeat_H, query_dim,value_dim, num_heads=1, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.k = nn.Linear(query_dim, query_dim, bias=qkv_bias)
        self.v = nn.Linear(query_dim, query_dim, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        ...
        self.up2 = nn.Linear(1, 1*3*3)
        self.upperfeat_H = upperfeat_H
        self.up2_2 = nn.Fold(output_size=(upperfeat_H, upperfeat_H), kernel_size=3, stride=2, padding=1)

    def forward(self, upperfeat, pre_pred_out):
        B, N, C = upperfeat.shape
        pre_pred_out = self.up2(pre_pred_out).transpose(1, 2)
        pre_pred_out = self.up2_2(pre_pred_out).reshape(B, -1, (self.upperfeat_H)*(self.upperfeat_H))
        vector = F.softmax(self.scale * pre_pred_out, dim=2)
        context = torch.matmul(vector, upperfeat)


        q = self.q(upperfeat).reshape(B, N, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        pred_tamper_feat = (attn @ v).transpose(1, 2).reshape(B, N, C)
        pred_tamper_feat = self.proj(pred_tamper_feat)
        pred_tamper_feat = self.proj_drop(pred_tamper_feat)

        pred_tamper_feat = pred_tamper_feat + upperfeat
...
class Net(nn.Module):
    def __init__(self, training=True):
        ...
        self.decoder2_to_1 = decoder_module(dim=64, dim2=64, img_size=self.img_size, ratio=1, kernel_size=(7, 7),
                                           stride=(4, 4),
                                           padding=(2, 2), fuse=False)
